...
def creer_un_vehicule(compteur_vehicules, voie, scenario, nb_voies, vitesse_limite):
    """
...
#PROBA CUMULEE : ORDONNER ?    
    ...
    while alea > proba_cumul:
        proba_cumul += p.PART_CONDUCT[scenario][i][1]
        i +=1
    conducteur = p.PART_CONDUCT[scenario][i-1][0]
    ...
    vitesse = vitesse_limite * conducteur.coef_vitesse
    ...
    if nb_voies != 1:
        proba_sortie = (nb_voies - voie.id_voie) / ((nb_voies*(nb_voies + 1))/2)
    else:
        proba_sortie = 1/5
    alea = random()
    ...
    i = 0
    while alea > proba_cumul:
        proba_cumul += p.PART_VEHICULE[scenario][i][1]
        i +=1
    
    
    #On cree le vehicule
    vehi = p.PART_VEHICULE[scenario][i-1][0](nom, conducteur, vitesse, prend_la_sortie, voie_vehi)
...
def generer_les_vehicules(compteur_vehicules, liste_voie, pas, debit, nb_vehicules_voulu, scenario, nb_voies, vitesse_limite):
    """
...
#COEF VEHI ?
            ...
            #cree)
            if vehi.position < 0.6*vitesse_limite:
    ...
    nb_voies_dispo = nb_voies - voie_occupee
    ...
    if nb_voies_dispo != 0:
        ...
        probabilite = debit * pas / nb_voies_dispo
    ...
    if probabilite != 0:
        ...
        for voie in liste_voie:
            ...
            if voie.libre:
                ...
                alea = random()
                if alea <= probabilite and compteur_vehicules != nb_vehicules_voulu:
                    nouveau_vehicule = creer_un_vehicule(compteur_vehicules, voie, scenario, nb_voies, vitesse_limite)
#DISTANCE A CREATION                    
#                    id_min = -1
#                    min_pos = 1200
